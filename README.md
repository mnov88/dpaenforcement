...
As of the moment, it only gathers information about DPA decisions in Norway.
...
```bash
# Quality Assessment (supports --input)
python data_quality_assessment.py --input dataNorway.csv

# Schema Validation (supports --input)
...
### Analysis Scripts
```bash
# Univariate analysis (defaults to latest cleaned)
